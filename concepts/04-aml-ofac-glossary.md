...
Interpose's AML pack is illustrative and uses public synthetic data; nothing here should be
treated as real regulatory guidance. See [[02-interpose-gateway-overview]] for why AML was
...
- **Sanctions / OFAC**: governments maintain lists of individuals, companies, and countries that
  ...
  against this list before transacting with them. Interpose's demo uses the *public* OFAC list.
...
- **SAR (Suspicious Activity Report)**: a formal report a financial institution is legally
  ...
  are legally sensitive documents; Interpose's demo produces an illustrative "investigation
...
- **Structuring**: deliberately breaking up a large transaction into many smaller ones to stay
  ...
  one of the heuristics Interpose's demo transaction-graph server implements
...
- **Beneficial owner**: the real human who ultimately owns or controls an entity (a shell
  ...
  ownership is a hard, deep problem in real AML work — explicitly *out of scope* for Interpose's
  demo (the scoping doc calls this out directly as something not to be tempted into building).

## Terms specific to how Interpose uses this domain
...
- **HITL (Human-In-The-Loop)**: a policy outcome where, instead of allow/deny, the call is
  *held* pending a human's explicit approval or denial. This is the concrete mechanism by which
  Interpose turns "an AI agent wants to take a real-world action in a regulated domain" into
...
- **Investigation agent**: the demo LangGraph agent that plays the role of a junior AML analyst
  ...
  Interpose, and produces a written assessment. It's the thing generating the ~40 tool calls per
  demo run that Interpose's policy engine and audit log get exercised against.
