...
| 22 | [langgraph-fundamentals-and-supervisor-routing.md](22-langgraph-fundamentals-and-supervisor-routing.md) | What LangGraph actually is (a state-machine library), typed state vs. chat transcripts, the Supervisor's two-hop routing (prose vs. diagram judgment call), why stub nodes for A2/A3/A4 |
| 23 | [control-plane-event-bus-and-feature-engineering.md](23-control-plane-event-bus-and-feature-engineering.md) | Decoupling the control plane from the gateway's hot path (the EventBus seam), Agent A1's live-queried features and the 3 deliberately-deferred ones, the risk-score heuristic, the first real slice of the deferred session-state hash |
...
| 29 | [embedded-analytics-with-duckdb.md](29-embedded-analytics-with-duckdb.md) | What an embedded OLAP engine is vs. Postgres, querying Parquet without loading it, the ephemeral write table + single-writer lock, k-hop BFS/induced-subgraph/window-anchoring design choices, and a real unpinned-dependency bug caught building the Docker image |
| 30 | [client-agents-vs-control-plane-agents.md](30-client-agents-vs-control-plane-agents.md) | Why the investigation agent is a *client* of Interpose, not part of it; a linear tool-calling graph vs. a ReAct loop; reusing the control plane's LLM wrapper on purpose; a real seed-generator OFFSET bug this project's own tests caught |
